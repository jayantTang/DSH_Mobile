...
iOS 客户端这一天的改动（TestFlight 构建 `20260922.1913` 起）：

- **本地缓存与增量同步**：会话列表快照、每会话转写尾部（200 条）与图片落盘；
  冷启动先显示上次的内容再增量合并，网络异常也能看；设置里可一键清除。
  缓存 30 天滚动、有容量上限，只存元数据与对话尾部。
- 会话列表新增「等你回应」状态（淡蓝底 + 左竖条），顶部徽标区分运行中与待回应。
