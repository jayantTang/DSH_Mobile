...
final class WorkspaceFileDownloaderTests: XCTestCase {
    ...
    private func downloader(_ host: Host) -> WorkspaceFileDownloader {
        WorkspaceFileDownloader(client: DSHClient(carrier: host), pacing: .zero)
    }
    ...
    func testFetchRemovesThePartialFileWhenTheSizeDoesNotAddUp() async throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: destination.path), "partial file left behind")
    }

    func testFetchStopsWhenAWindowIsEmptyWithoutEOF() async throws {
        ...
        do {
            ...
            XCTAssertEqual(failure, .stalled(offset: 0))
        }
        XCTAssertEqual(host.calls.filter { $0.method == "workspaceFiles/readBytes" }.count, 1)
        XCTAssertFalse(FileManager.default.fileExists(atPath: destination.path))
    }

    func testCancellationLeavesNoFileBehind() async throws {
